...
\begin{DoxyVerb}update the state
...
\begin{DoxyVerb}Callback function of odometry
...
\begin{DoxyVerb}stop the robot and set the robot linear and angluar vel to 0\end{DoxyVerb}
...
\begin{DoxyVerb}orient the robot toward the final yaw
...
\begin{DoxyVerb}orient the robot in the current position
...
\begin{DoxyVerb}move the robot toward the goal
...
\begin{DoxyVerb}state machine: depending on the state, change the behaviour of the robot until goal is preempted.
...
\begin{DoxyVerb}main function: define the publisher and subscriber
\end{DoxyVerb}
...
\begin{DoxyVerb}normalize the angle between [-pi, pi]
